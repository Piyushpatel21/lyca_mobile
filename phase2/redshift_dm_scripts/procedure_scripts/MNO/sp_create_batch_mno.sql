CREATE OR REPLACE PROCEDURE uk_logs.sp_create_batch_mno(p_rundate timestamp,p_batch_count numeric,p_source varchar,p_schema varchar)
	LANGUAGE plpgsql
AS $$
...
BEGIN
	...
	INTO v_max_batchid
	FROM log_batch_files_mno;--log_batch_files_rrbs;
...
*/
	...
	SELECT COUNT(1)
	INTO v_check_batch
	FROM log_batch_files_mno--log_batch_files_rrbs--_new --log_batch_files_rrbs 
	...
	BEGIN
		...
		--insert
		INSERT INTO log_batch_files_mno--log_batch_files_rrbs --_new--log_batch_files_rrbs_1
			(
			batch_id
			,file_source
			,file_id
