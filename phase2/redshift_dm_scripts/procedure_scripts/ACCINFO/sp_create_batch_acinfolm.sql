CREATE OR REPLACE PROCEDURE uk_logs.sp_create_batch_acinfolm(p_rundate timestamp,p_batch_count numeric,p_source varchar,p_schema varchar)
	LANGUAGE plpgsql
AS $$
...
BEGIN
	...
	INTO v_max_batchid
	FROM log_batch_files_acinfolm;--log_batch_files_rrbs;
...
*/
	...
	SELECT COUNT(1)
	INTO v_check_batch
	FROM log_batch_files_acinfolm--log_batch_files_rrbs--_new --log_batch_files_rrbs 
	...
	BEGIN
		IF v_check_batch > 0 THEN RAISE NOTICE 'Batch ''%'' already created'
			,v_start_time;ELSE
			lock log_batch_files_acinfolm;--log_batch_files_rrbs;

		--insert
		INSERT INTO log_batch_files_acinfolm--log_batch_files_rrbs --_new--log_batch_files_rrbs_1
			(
			batch_id
			,file_source
			,file_id
