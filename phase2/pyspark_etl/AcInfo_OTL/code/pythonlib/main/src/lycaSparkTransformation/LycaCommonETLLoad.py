...
class LycaCommonETLLoad:
    ...
    def __init__(self, module, submodule, configfile, connfile, master, run_date=None, batchID= None, source_file_path=None):
...
def start_execution(args):
    ...
    tf = TransformActionChain(sparkSession, logger, args.get('module'), args.get('submodule'), configfile, connfile,
                              run_date, prevDate, args.get('source_file_path'))
    if not (args.get('run_date') and args.get('batchID')):
