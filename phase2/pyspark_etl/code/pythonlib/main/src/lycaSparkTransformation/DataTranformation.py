...
class DataTranformation:
    ...
    @staticmethod
    def getLateOrNormalCdr(dataFrame: DataFrame, dateColumn, formattedDateColumn, integerDateColumn, dateRange) -> DataFrame:
        df_event_date = dataFrame.withColumn(integerDateColumn, py_function.substring(py_function.col(dateColumn), 0, 8).cast(IntegerType()))
        df_normalOrLate = df_event_date.withColumn("normalOrlate", py_function.when(py_function.col(integerDateColumn) < int(dateRange), "Late").otherwise("Normal"))\
                                       .withColumn(formattedDateColumn, py_function.to_date(py_function.to_date(py_function.col(integerDateColumn).cast(StringType()), 'yyyyMMdd'), 'yyyy-MM-dd'))
