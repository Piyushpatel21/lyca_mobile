...
class TransformActionChain:
    ...
    def getSourceData(self, batchid, srcSchema, checkSumColumns) -> Tuple[DataFrame, DataFrame, DataFrame, DataFrame]:
        ...
        try:
            ...
            metaQuery = ("INSERT INTO uk_test.log_batch_status_rrbs (BATCH_ID, S3_BATCHREADCOUNT, S3_FILECOUNT, BATCH_STATUS, BATCH_START_DT) values({batch_id},{s3_batchreadcount},{s3_filecount},'{batch_status}','{batch_start_dt}')"
            ...
            metaQuery = ("update uk_test.log_batch_status_rrbs set INTRABATCH_DEDUPL_STATUS='Complete', INTRABATCH_DUPL_COUNT={intrabatch_dupl_count}, BATCH_STATUS='{batch_status}', INTRABATCH_DIST_DUPL_COUNT={intrabatch_dist_dupl_count} where BATCH_ID={batch_id} and BATCH_END_DT is null"
            ...
            metaQuery = ("update uk_test.log_batch_status_rrbs set INTRABATCH_NEW_LATE_COUNT={intrabatch_late_count} where batch_id={batch_id} and batch_end_dt is null".format(batch_id=batchid, intrabatch_late_count=''.join(str(e) for e in intrabatch_late_count)))
            self.redshiftprop.writeBatchStatus(self.sparkSession, metaQuery)
            ...
            metaQuery = ("update uk_test.log_batch_status_rrbs set INTRABATCH_NEW_NORMAL_COUNT={intrabatch_new_count}, INTRABATCH_DEDUPL_STATUS='{intrabatch_status}' where BATCH_ID={batch_id} and BATCH_END_DT is null".format(batch_id=batchid, intrabatch_status=intrabatch_status, intrabatch_new_count=''.join(str(e) for e in intrabatch_new_count)))
            self.redshiftprop.writeBatchStatus(self.sparkSession, metaQuery)
            ...
            metaQuery = ("INSERT INTO uk_test.log_batch_status_rrbs (batch_id,batch_status, batch_start_dt, batch_end_dt) values({batch_id}, '{batch_status}', '{batch_start_dt}', '{batch_end_dt}')".format(batch_id=batchid, batch_status='Failed', batch_start_dt=self.batch_start_dt, batch_end_dt=datetime.now()))
            self.redshiftprop.writeBatchStatus(self.sparkSession, metaQuery)
    ...
    def getLateCDR(self, srcDataFrame: DataFrame, lateDBDataFrame: DataFrame, batchid) -> Tuple[DataFrame, DataFrame, DataFrame, DataFrame]:
        try:
            ...
            metaQuery = ("update uk_test.log_batch_status_rrbs set LDM_LATECDR_STATUS='{ldm_latecdr_status}', LDM_LATECDR_COUNT={ldm_latecdr_count}, LDM_LATECDR_DBDUPL_COUNT={ldm_latecdr_dupl_count} where BATCH_ID={batch_id} and BATCH_END_DT is null"
    ...
    def getNormalCDR(self, srcDataFrame: DataFrame, normalDBDataFrame: DataFrame, batchid) -> Tuple[DataFrame, DataFrame, DataFrame, DataFrame]:
        try:
            ...
            metaQuery = ("update uk_test.log_batch_status_rrbs set DM_NORMAL_STATUS='{dm_normal_status}', DM_NORMAL_COUNT={dm_normal_count}, DM_NORMAL_DBDUPL_COUNT={dm_normal_dupl_count} where BATCH_ID={batch_id} and BATCH_END_DT is null"
    ...
    def writeBatchStatus(self, batch_id, status):
        ...
        batch_status = status
        metaQuery = ("update uk_test.log_batch_status_rrbs set BATCH_STATUS='{batch_status}', BATCH_END_DT='{batch_end_dt}' where BATCH_ID = {batch_id} and BATCH_END_DT is null"
