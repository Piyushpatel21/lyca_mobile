...
class TestAgg1RRBSTopUp:

    sms_fact_location = 'assets/fact_rrbs_topup_2020-07-22.csv'
    ...
    def test_get_aggregation_instance(self, set_up):
        args = {
            ...
            "module": "rrbs",
            "sub_module": "topup",
            "configfile": "../configs/agg1_rrbs_topup_application_properties.json",
        ...
        # Read data
        df = aggregator.spark.read.csv(self.sms_fact_location, header=True)
        ...
        print("Hourly: ", df_distinct_groupby_hour.count() if df_distinct_groupby_hour else None)

        print("Daily: ", df_distinct_groupby_daily.count() if df_distinct_groupby_daily else None)

        print("Monthly: ", df_distinct_groupby_monthly.count() if df_distinct_groupby_monthly else None)
        print(df_distinct_groupby_monthly.show())

        print("Yearly: ", df_distinct_groupby_yearly.count() if df_distinct_groupby_yearly else None)
