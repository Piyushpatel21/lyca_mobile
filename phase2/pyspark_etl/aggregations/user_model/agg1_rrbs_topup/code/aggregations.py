"""
Perform Aggregation on RRBS TopUp Data. It performs:
* Read from Redshift
* Calculate Metrics
* Save to Redshift
"""
...
class Aggregation:
    ...
    def __init__(self, spark, module, sub_module, creation_date_range, start_date, end_date, fmt, conn_file_path, config_file_path, code_bucket, logger):
        ...
        self.src_cols = ['msisdn', 'bundle_code', 'imsi', 'bundle_name', 'network_id', 'cdr_dt_month',
                         'cdr_dt', 'cdr_dt_num', 'cdr_dt_hour_gmt']
        self.hourly_cols = ['msisdn', 'user_type', 'bundle_code', 'imsi', 'bundle_name', 'network_id', 'cdr_dt_hour_gmt',
                            'cdr_dt', 'cdr_dt_num', 'cdr_dt_month', 'cdr_dt_year']
        self.daily_cols = ['msisdn', 'user_type', 'bundle_code', 'imsi', 'bundle_name', 'network_id',
                           'cdr_dt', 'cdr_dt_num', 'cdr_dt_month', 'cdr_dt_year']
        self.monthly_cols = ['msisdn', 'user_type', 'bundle_code', 'imsi', 'bundle_name', 'network_id',
                             'cdr_dt_month', 'cdr_dt_year']
        self.yearly_cols = ['msisdn', 'user_type', 'bundle_code', 'imsi', 'bundle_name', 'network_id',
                            'cdr_dt_year']
    ...
    def perform_distinct_aggregation(self, df: DataFrame, hourly_df=None, daily_df=None, monthly_df=None, yearly_df=None):
        ...
        if self.fmt.startswith('%Y%m') or self.creation_date_range:
            df_distinct_groupby_monthly = self.spark.sql(rrbs_topup_monthly.format(table=self.spark_table_name))
