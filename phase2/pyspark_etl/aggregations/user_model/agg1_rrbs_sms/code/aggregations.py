"""
Perform Aggregation on RRBS SMS Data. It performs:
* Read from Redshift
* Calculate Metrics
* Save to Redshift
"""
...
class Aggregation:
    ...
    def read_agg_tables(self):
        ...
        if self.fmt.startswith('%Y%m%d') or self.creation_date_range:
            ...
            query = "SELECT {cols} FROM {table} WHERE {msg_date} >= {start} AND {msg_date} <= {end}".format(
                cols=','.join(self.hourly_cols), table=full_hourly_tbl_name, start=self.start, end=self.end,
                msg_date='msg_date_num'
            )
        ...
        if self.fmt.startswith('%Y') or self.creation_date_range:
            ...
            query = "SELECT {cols} FROM {table} WHERE {msg_year} >= {start} AND {msg_year} <= {end}".format(
                cols=','.join(self.yearly_cols), table=full_yearly_tbl_name, start=self.start[:4],
                end=self.end[:4],
                msg_year='msg_date_year'
            )
