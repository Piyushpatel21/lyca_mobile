"""
Perform Aggregation on RRBS Voice Data. It performs:
* Read from Redshift
* Calculate Metrics
* Save to Redshift
"""
...
class Aggregation:
    ...
    def read_agg_tables(self):
        ...
        if self.fmt.startswith('%Y%m%d') or self.creation_date_range:
            ...
            query = "SELECT {cols} FROM {table} WHERE {call_date} >= {start} AND {call_date} <= {end}".format(
                cols=','.join(self.hourly_cols), table=full_hourly_tbl_name, start=self.start, end=self.end,
                call_date='call_date_num'
            )
        ...
        if self.fmt.startswith('%Y%m') or self.creation_date_range:
            ...
            query = "SELECT {cols} FROM {table} WHERE {call_month} >= {start} AND {call_month} <= {end}".format(
                cols=','.join(self.monthly_cols), table=full_monthly_tbl_name, start=self.start[:6], end=self.end[:6],
                call_month='call_date_month'
            )
        ...
        if self.fmt.startswith('%Y') or self.creation_date_range:
            ...
            query = "SELECT {cols} FROM {table} WHERE {call_year} >= {start} AND {call_year} <= {end}".format(
                cols=','.join(self.yearly_cols), table=full_yearly_tbl_name, start=self.start[:4],
                end=self.end[:4],
                call_year='call_date_year'
            )
