"""
Perform Aggregation on RRBS GPRS Termination Data. It performs:
* Read from Redshift
* Calculate Metrics
* Save to Redshift
"""
...
class Aggregation:
    ...
    def __init__(self, spark, module, sub_module, creation_date_range, start_date, end_date, fmt, conn_file_path, config_file_path, code_bucket, logger):
        ...
        self.src_cols = ['msisdn', 'bundle_code', 'data_feature', 'tariffplan_id', 'cdr_type', 'network_id', 'roam_flag', 'data_termination_hour',
                         'data_termination_dt', 'data_termination_dt_num', 'data_termination_month']
        ...
        self.monthly_cols = ['msisdn', 'user_type', 'bundle_code', 'tariffplan_id', 'cdr_type', 'network_id', 'roam_flag', 'data_feature',
                             'data_termination_month', 'data_termination_year']
    ...
    def read_agg_tables(self):
        ...
        if self.fmt.startswith('%Y%m%d') or self.creation_date_range:
            ...
            query = "SELECT {cols} FROM {table} WHERE {gprs_term_date} >= {start} AND {gprs_term_date} <= {end}".format(
                cols=','.join(self.hourly_cols), table=full_hourly_tbl_name, start=self.start, end=self.end,
                gprs_term_date='data_termination_dt_num'
            )
        ...
        if self.fmt.startswith('%Y%m') or self.creation_date_range:
            ...
            query = "SELECT {cols} FROM {table} WHERE {gprs_term_month} >= {start} AND {gprs_term_month} <= {end}".format(
                cols=','.join(self.monthly_cols), table=full_monthly_tbl_name, start=self.start[:6], end=self.end[:6],
                gprs_term_month='data_termination_month'
            )
        ...
        if self.fmt.startswith('%Y') or self.creation_date_range:
            ...
            query = "SELECT {cols} FROM {table} WHERE {gprs_term_year} >= {start} AND {gprs_term_year} <= {end}".format(
                cols=','.join(self.yearly_cols), table=full_yearly_tbl_name, start=self.start[:4],
                end=self.end[:4],
                gprs_term_year='data_termination_year'
            )
    ...
    def perform_distinct_aggregation(self, df: DataFrame, hourly_df=None, daily_df=None, monthly_df=None, yearly_df=None):
        ...
        if self.fmt.startswith('%Y%m%d') or self.creation_date_range:
            ...
            df_distinct_groupby_hourly = self.spark.sql(rrbs_gprs_term_hourly.format(table=self.spark_table_name))
            if df_distinct_groupby_hourly and hourly_df:
                df_distinct_groupby_hourly = df_distinct_groupby_hourly.subtract(hourly_df)

            df_distinct_groupby_daily = self.spark.sql(rrbs_gprs_term_daily.format(table=self.spark_table_name))
            if df_distinct_groupby_daily and daily_df:
                df_distinct_groupby_daily = df_distinct_groupby_daily.subtract(daily_df)

        if self.fmt.startswith('%Y%m') or self.creation_date_range:
            df_distinct_groupby_monthly = self.spark.sql(rrbs_gprs_term_monthly.format(table=self.spark_table_name))
            if monthly_df and df_distinct_groupby_monthly:
                df_distinct_groupby_monthly = df_distinct_groupby_monthly.subtract(monthly_df)

        if self.fmt.startswith('%Y') or self.creation_date_range:
            df_distinct_groupby_yearly = self.spark.sql(rrbs_gprs_term_yearly.format(table=self.spark_table_name))
